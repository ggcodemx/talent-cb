...
async function getLiderazgoData() {
  try {
    const payload = await getPayload({ config: configPromise })
    return await payload.findGlobal({ slug: 'page-liderazgo' })
  } catch (error) {
    console.error('Error fetching page-liderazgo:', error)
    return null
  }
}

export default async function LiderazgoPage() {
  const page = await getLiderazgoData()
      ...
      ? {
          cardLeft: {
            ...
            image: {
              ...
              alt: typeof pageAny.flipCards.cardLeft.image === 'object'
          ...
          cardRight: {
            ...
            image: {
              ...
              alt: typeof pageAny.flipCards.cardRight.image === 'object'
  ...
  return (
    <>
      <PageHero
        title={page?.hero?.title ?? 'Liderazgo responsable'}
        description={page?.hero?.description ?? ''}
      />
      {splitData && <LiderazgoSplit {...splitData} />}
