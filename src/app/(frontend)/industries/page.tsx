...
async function getIndustriesData() {
  try {
    ...
    return await payload.findGlobal({ slug: 'page-industries' })
  } catch (error) {
    console.error('Error fetching page-industries:', error)
    return null
  }
}

export default async function Industries() {
  const page = await getIndustriesData()

  const industriesData: IndustriesBlockProps | null = page?.industries?.length
    ? {
        industries: page.industries.map((ind: any) => ({
          name: ind.name,
  ...
  return (
    <>
      <PageHero
        title={page?.hero?.title ?? 'Industrias'}
